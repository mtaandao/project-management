            ...
            <div class="cpm-col-2 cpm-sm-col-12 cpm-user-select">
                <?php
                    if(isset( $_GET['page'] ) && $_GET['page'] === 'cpm_task') {
                        if ( cpm_can_manage_projects() ) { ?>
                        <select class="cpm-mytask-switch-user" data-tab="<?php echo $ctab ?>">
                            <option><?php _e( 'Select an User', 'cpm' ) ?></option>
                        <?php
                        foreach (get_users(array('order' => 'ASC')) as $suser) {
                            ?>
                                <option value="<?php echo $suser->ID ?>"<?php if ( $user_id == $suser->ID ) echo ' selected="selected"'; ?>><?php echo $suser->display_name ?></option>
                                <?php
                            }
                            ?>
                        </select>
